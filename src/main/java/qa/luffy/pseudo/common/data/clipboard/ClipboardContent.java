...
public record ClipboardContent(String title, int active, List<Page> pages) {
    ...
    public record Page(List<CheckboxState> checkboxes, List<String> lines) {
        public Page(List<CheckboxState> checkboxes, List<String> lines) {
            this.checkboxes = extend(checkboxes, MAX_LINES, CheckboxState.EMPTY);
            this.lines = extend(lines, MAX_LINES, "");
        }
        ...
        private static <T> List<T> extend(List<T> in, int size, T fill) {
            List<T> out = new ArrayList<>(in == null ? List.<T>of() : in);
            if (out.size() > size) {
